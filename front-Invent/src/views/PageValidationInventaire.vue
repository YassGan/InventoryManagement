...
export default {
  ...
  methods: {
    ...
    updateSingleProduct(_id, ordre) {
      ...
      alert("article cliqué avec la souris " + _id);

      alert("new quqantities " + q1v + q2v + q3v);

      HTTP.put(`/ArticlesAPI/updateWrongArticleComptage/${_id}`, {
        statut: 5,
        quantite1: q1v,
        quantite2: q2v,
        quantite3: q3v,
    ...
    async getArticlesDynamically() {
      ...
      const AllArticles = await HTTP.get(
        `ArticlesAPI/allArticlesByLienArticleInventaire/${InventId}`
      );
...
//cette requête change le statut de l'inventaire de inventaire nonArchivé à inventaireArchivé
        HTTP.put(
          `/Inventaires_non_verifiesAPI/updateInventoryStatut/${InventId}`
        )
          .then((response) => {
